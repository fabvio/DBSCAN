  ...
  , labels(dset->rows(), -1)
{
  ...
  LOG(INFO) << "IL TIPO " << typeid(dset->rows()).name() << " il NUMERO " << dset->rows();
...
{
  LOG(INFO) << h_Va0.size() << "CIO" << h_Va1.size() << " " << vA_size << " ajkshdaks " << (h_Va0[h_Va0.size() - 1]) << "jj" << ( h_Va1[h_Va1.size() - 1]);
...
GDBSCAN_RT::GDBSCAN_RT(const unsigned long max_numbers, const unsigned long max_points)
  : m_dset(boost::make_shared< Dataset >())
  , d_data(0)
  , vA_max_size(sizeof(int) * max_points)
  , vA_size(0)
  , d_Va0(0)
  , d_Va1(0)
  , d_Ea(0)
  ...
  , m_predict_time(.0)
{
  ...
  size_t alloc_size = sizeof(float) * max_numbers;
  cudaError_t r = cudaMalloc(reinterpret_cast<void**>(&d_data), alloc_size);
  ...
  r = cudaMalloc(reinterpret_cast<void**>(&d_Va0), vA_max_size);

  if (r != cudaSuccess) {
    throw std::runtime_error("Cuda d_Va0 malloc error :" + std::to_string(r));
  }

  r = cudaMalloc(reinterpret_cast<void**>(&d_Va1), vA_max_size);

  if (r != cudaSuccess) {
    throw std::runtime_error("Cuda d_Va1 malloc error :" + std::to_string(r));
  }

  LOG(INFO) << "Allocated " << vA_max_size << " bytes on device for Va0 and Va1";

  r = cudaMalloc(reinterpret_cast<void**>(&d_Fa), vA_max_size);

  if (r != cudaSuccess) {
    throw std::runtime_error("Cuda d_Fa malloc error :" + std::to_string(r));
  }

  LOG(INFO) << "Allocated " << vA_max_size << " bytes on device for d_Fa";

  r = cudaMalloc(reinterpret_cast<void**>(&d_Xa), vA_max_size);
  ...
  r = cudaMalloc(reinterpret_cast<void**>(&d_Ea), Ea_size);
...
void
GDBSCAN_RT::fit(Dataset::Ptr dset, float eps, size_t min_elems)
{
  m_dset = dset;
  vA_size = sizeof(int) * dset->rows();
  h_Va0 = std::vector<int>(dset->rows(), 0);
  h_Va1 = std::vector<int>(dset->rows(), 0);

  core = std::vector<bool>(dset->rows(), false);
  labels = Labels(dset->rows(), -1);

  const size_t cols = m_dset->cols();
  size_t copysize = cols * sizeof(float);

  for (size_t i = 0; i < m_dset->rows(); ++i) {
    cudaError_t r = cudaMemcpy(d_data + i * cols,
                   m_dset->data()[i].data(),
                   copysize,
                   cudaMemcpyHostToDevice);
...
void
GDBSCAN_RT::Fa_Xa_to_device(const std::vector<int>& Fa, const std::vector<int>& Xa)
{
  LOG(INFO) << "jdjas" << Fa[0];  
  cudaError_t r = cudaMemcpy(d_Fa, &Fa[0], vA_size, cudaMemcpyHostToDevice);
  LOG(INFO) << "jdjas" << Fa[0];
...
void
GDBSCAN_RT::Xa_to_host(std::vector<int>& Xa)
{
  cudaError_t r = cudaMemcpy(&Xa[0], d_Xa, vA_size, cudaMemcpyDeviceToHost);
  if (r != cudaSuccess) {
    throw std::runtime_error("Cuda memcpy Xa device to host :" +
...
void
GDBSCAN_RT::Fa_to_host(std::vector<int>& Fa)
{
  cudaError_t r = cudaMemcpy(&Fa[0], d_Fa, vA_size, cudaMemcpyDeviceToHost);
  if (r != cudaSuccess) {
    throw std::runtime_error("Cuda memcpy Fa device to host :" +
...
void
GDBSCAN_RT::breadth_first_search(int i,
                              int32_t cluster,
                              std::vector<bool>& visited)
{
  int N = static_cast<int>(m_dset->rows());
...
const GDBSCAN_RT::Labels&
GDBSCAN_RT::get_labels()
{
  return labels;
}
